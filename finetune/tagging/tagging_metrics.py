...
class SpanEntityLevelF1Scorer(F1Scorer):
  """Computes F1 score for entity-level tasks such as NER."""

  def __init__(self, label_mapping):
    super(SpanEntityLevelF1Scorer, self).__init__()
    self._inv_label_mapping = {v: k for k, v in six.iteritems(label_mapping)}
  ...
  def _get_results(self):
    ...
    for labels, preds in zip(self._labels, self._preds):
      ...
      self._n_gold += len(sent_spans)
      self._n_predicted += len(span_preds)
    return super(SpanEntityLevelF1Scorer, self)._get_results()

class NERTokenLevelF1Scorer(F1Scorer):
  """Computes F1 score for entity-level tasks such as NER."""

  def __init__(self, label_mapping):
    super(NERTokenLevelF1Scorer, self).__init__()
    self._label_mapping = label_mapping
    self._inv_label_mapping = {v: k for k, v in six.iteritems(label_mapping)}

  def _get_results(self):
    self._n_correct, self._n_predicted, self._n_gold = 0, 0, 0
    idx_O = self._label_mapping['O']
    for labels, preds in zip(self._labels, self._preds):
      for label, pred in zip(labels, preds):
        ...
        if label != idx_O:
          self._n_gold += 1
    return super(NERTokenLevelF1Scorer, self)._get_results()